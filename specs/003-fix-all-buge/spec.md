...
**Feature Branch**: `003-fix-all-buge`

**Created**: 2026-06-05

**Status**: Draft

**Input**: User description: "fix all buge"
...
2. **Given** a verified account, **When** the user uploads a PDF knowledge document, **Then** the document is processed (virus scan → extraction → embedding) and marked as "Ready" within 2 minutes per megabyte of file size.
...
3. **Given** a URL scraping request, **When** the admin provides a valid website URL, **Then** the system extracts and indexes the page content as a knowledge source (up to a depth of 1 and a cap of 50 total pages per tenant).
...
3. **Given** a resolved escalation, **When** the agent marks it closed, **Then** the conversation is archived and a Customer Satisfaction (CSAT) rating prompt is triggered to the customer in the chat widget.
...
2. **Given** a hallucination report, **When** the admin reviews it, **Then** they can flag the knowledge source causing the issue and trigger re-processing / re-indexing of that specific source file.
...
- **FR-002**: System MUST send an email verification link upon registration and prevent full platform access (redirect to verification page) until email is confirmed.
...
- **FR-007**: Each tenant's data MUST be strictly isolated at rest. Row Level Security (RLS) is mandatory in PostgreSQL. Vector embeddings MUST use separate Qdrant collections per tenant. S3/MinIO storage and Redis cache MUST use tenant-prefixed folders and key namespaces.
- **FR-007b**: Tenants MUST select a data residency region at signup (GCC region or Global). All tenant data — conversations, knowledge, audit logs — MUST be stored exclusively in the selected region for the lifetime of the workspace, strictly enforced at the database connection and network routing levels.
...
- **FR-009**: System MUST enforce guardrails: restrict external knowledge by default, with optional competitor blocking (domain filtering matching lists defined by the admin) and configurable escalation rules (based on message threshold or low confidence threshold).

**Knowledge Management**

- **FR-010**: System MUST accept knowledge uploads in PDF, DOCX, TXT, CSV, and XLSX formats, and support URL scraping (restricted to a maximum depth of 1 and cap of 50 total pages) and manual Q&A entry.
...
- **FR-015**: System MUST apply context compression and token optimization before passing context to the LLM by pruning segments below a threshold of 0.75 cosine similarity.
...
- **FR-022**: Admins MUST be able to provide a correct answer for any failed question, which is then embedded into the knowledge base and the question marked Resolved. Admins MUST be able to flag the specific knowledge source file causing the hallucination to trigger a re-processing event.
...
- **FR-026**: When a tenant reaches their monthly message limit, the system MUST continue serving AI conversations without interruption (soft allow). Overage messages are metered in real-time and charged automatically at the end of the billing cycle. The Tenant Admin MUST receive an in-app and email alert when usage crosses 80% and again at 100% of their plan limit.

**Notifications Engine**

- **FR-027**: System MUST deliver notifications via email and in-app channels for events: complaint received, lead created, subscription expiring, document processing failed, agent assigned, and usage limits (80% and 100% threshold notifications).
...
- **FR-033**: Tenant Admins MUST be able to export conversation history, lead records, and audit logs in CSV and JSON formats on demand. Exports MUST be triggered via explicit download buttons on the settings dashboard, and exports MUST be scoped to the tenant's own data only. Knowledge base document export is out of scope for Phase 1.

**Security & RBAC**

- **FR-029**: System MUST enforce role-based access control with four roles — Super Admin, Tenant Admin, Agent, Viewer — each with strictly defined permission scopes. Navigation links, buttons, settings pages, and forms MUST be conditionally rendered (hidden or disabled) on the frontend depending on the authenticated user's role. Super Admin access is restricted to platform-level operations: tenant management, billing administration, and system health. Super Admin MUST NOT have routine read access to tenant conversation data. Emergency access to tenant data (e.g., for critical support) MUST require an explicit break-glass procedure that is recorded as an immutable audit log entry visible to the Tenant Admin.
...
- The admin dashboard and agent interface must support full Arabic/RTL layout in Phase 1 alongside English/LTR.
...
- Microsoft OAuth and SMS notifications are out of scope for Phase 1.
- Only Stripe is required for Phase 1.
